...
for (scale in scales){
  ...
  Bembix_reg_raw <- read.csv("../data/Regpoints_04062020.csv")
  
  Bembix_raw <- Bembix_raw %>% rename("NestID"="ID")
  #only get the values on 2m
  Bembix_scale <- Bembix_raw[,c('Presence', 'X', 'Y', "NestID", "RealNest",
                                paste0("NDVI", scale, "mean"), 
                                paste0("W", scale, "mean"),
                                paste0("Sl", scale, "mean"))]
  colnames(Bembix_scale) <- c("Presence", "X", "Y", "NestID", "RealNest", "NDVImean", "Wmean", "Slmean")
  site <- factor(rep("a",nrow(Bembix_scale))) 
  Bembix <-  cbind(Bembix_scale, site)
  ### REAL NESTS
  ...
  Bembix$NDVIstd <- standard_score(Bembix$NDVImean)
  Bembix$Wstd    <- standard_score(Bembix$Wmean)
  Bembix$Slstd   <- standard_score(Bembix$Slmean)
  
  #formatting dataset regular points for predictions
  Bembix_reg <- Bembix_reg_raw[,c('X', 'Y', paste0("NDVI", scale, "mean"), paste0("W", scale, "mean"))]
  colnames(Bembix_reg) <- c("X", "Y", "NDVImean", 'Wmean')
  Presence = rep(NA, nrow(Bembix_reg_raw))
  Bembix_reg <- cbind(Bembix_reg, Presence)
  
  ###Standardisation (with the origianl dataset as reference!)
  Bembix_reg$NDVIstd <- (Bembix_reg$NDVImean - mean(Bembix$NDVImean))/sd(Bembix$NDVImean)
  Bembix_reg$Wstd    <- (Bembix_reg$Wmean-mean(Bembix$Wmean))/sd(Bembix$Wmean)
  
  Bembix_reg <- Bembix_reg %>% drop_na(c(NDVIstd, Wstd))
  ...
  it_numbers <- sample(1:999, 5, replace=F)
  ...
  for (it_number in it_numbers){
    ...
    #save model in rda
    save(M, file = paste0("./output_iterations/scale_", scale, "v2/M", it_number, "-", Sys.Date(), ".rda"))
    #save mesh
    save(mesh, file= paste0("./output_iterations/scale_", scale, "v2/mesh", it_number, "-", Sys.Date(), ".rda"))
  ...
  # effect sizes -> per model
  write.table(effect_sizes, paste0("./output_iterations/scale_", scale,"v2/effect_sizes.txt"), sep="\t")
  # variogramdata -> separate file
  write.table(variogramsV0_df, paste0("./output_iterations/scale_", scale,"v2/variogramsV0.txt"), sep="\t")
  write.table(variogramsV1_df, paste0("./output_iterations/scale_", scale,"v2/variogramsV1.txt"), sep="\t")
  # spatial field -> models and meshes are saved during iterations
  # posterior distributions -> separate file
  write.table(posteriors_df, paste0("./output_iterations/scale_", scale,"v2/posterior_distributions.txt"), sep="\t")
  # predictions for 30% test-data -> add to file with for each prediction-set X, Y, prediction. Each combination of X&Y --> unique point
  write.table(Bembix_30_pred, paste0("./output_iterations/scale_", scale,"v2/predictions_testdata.txt"), sep="\t")
  #AUC
  write.table(Bembix_auc, paste0("./output_iterations/scale_", scale,"v2/auc.txt"), sep="\t")
  #suitability map predictions
  write.table(Predictions_map_df, paste0("./output_iterations/scale_", scale,"v2/predictions_reg.txt"), sep="\t")
}
