...
for (scale in scales){
  ...
  it_numbers <- sample(1:999, 1, replace=F)
  ...
  for (it_number in it_numbers){
    ...
    #save model in rda
    save(M, file = paste0("./output_iterations/scale_", scale, "/M", it_number, "-", Sys.Date(), ".rda"))
    #save mesh
    save(mesh, file= paste0("./output_iterations/scale_", scale, "/mesh", it_number, "-", Sys.Date(), ".rda"))
  ...
  # effect sizes -> per model
  write.table(effect_sizes, paste0("./output_iterations/scale_", scale,"/effect_sizes.txt"), sep="\t")
  # variogramdata -> separate file
  write.table(variogramsV0_df, paste0("./output_iterations/scale_", scale,"/variogramsV0.txt"), sep="\t")
  write.table(variogramsV1_df, paste0("./output_iterations/scale_", scale,"/variogramsV1.txt"), sep="\t")
  # spatial field -> models and meshes are saved during iterations
  # posterior distributions -> separate file
  write.table(posteriors_df, paste0("./output_iterations/scale_", scale,"/posterior_distributions.txt"), sep="\t")
  # predictions for 30% test-data -> add to file with for each prediction-set X, Y, prediction. Each combination of X&Y --> unique point
  write.table(Bembix_30_pred, paste0("./output_iterations/scale_", scale,"/predictions_testdata.txt"), sep="\t")
  #AUC
  write.table(Bembix_auc, paste0("./output_iterations/scale_", scale,"/auc.txt"), sep="\t")
  #suitability map predictions
  write.table(Predictions_map_df, paste0("./output_iterations/scale_", scale,"/predictions_reg.txt"), sep="\t")
}
