...
for scenario in ['Random', 'UNIFORM', 'FIXED', 'FLEXIBLE']:#iteration-numbers will be done for every submodel
    for i in range(begin_int, end_int):
        ...
        range_ca = np.random.uniform(0.2, 7)#range at which nests are taken into account; here uniform distribution between 0.5-20m?
        param_mindens_ca = np.random.uniform(-10, -3) #chance at zero density is then 1/(1+exp(-params_mindens_ca)); here -10,+10
        ...
        pop.let_it_run()
        pop.create_output(i, savepath='data/Outputs example/official_run{}'.format(str(it_number)))
        #tk.mainloop()
